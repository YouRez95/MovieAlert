...
export const aboutMovieHandler = catchErrors(async (req, res) => {
  // validate title
  const { movieId, movieName } = aboutMovieSchema.parse({
...
export const myMoviesHandler = catchErrors(async (req, res) => {
  ...
  const movies = await MovieModel.find({ userId: req.userId })
    .sort({ createdAt: -1 })
    .limit(maxMovies)
    .skip((page - 1) * maxMovies)
    .select("_id picture title");
...
export const myViewsHandler = catchErrors(async (req, res) => {
  ...
  // Get the documents
  const countMovies = await SearchLogModel.countDocuments({
    userId: req.userId,
  });
  const movies = await SearchLogModel.find({ userId: req.userId })
    .sort({ createdAt: -1 })
    .limit(maxMovies)
    .skip((page - 1) * maxMovies)
    .select("movieId")
    .populate("movieId", "_id picture title");
